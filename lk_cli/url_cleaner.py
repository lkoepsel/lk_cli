...
TRACKING_PARAMS = {
    ...
    # Amazon
    'tag', 'linkCode', 'linkId', 'ref_', 'ref', 'psc',
    
    # Other common tracking parameters
    'igshid', 'gs_l', '_ga', 'gws_rd', 'ei', 'ved', 'uact',
    'sa', 'source', 'medium', 'campaign', 'content', 'term',
    'affiliate_id', 'aff_id', 'partner_id', 'referrer',
    'click_id', 'clickid', 'transaction_id', 'tid',
    ...
    'hid', 'did', 'lctg', 'lr_input',  # Common email tracking
    'newsletter_id', 'email_id', 'subscriber_id', 'list_id',
    'delivery_id', 'message_id', 'campaign_id', 'send_id',
    'recipient_id', 'hash_id', 'tracking_id', 'unique_id',
    'email_hash', 'subscriber_hash', 'delivery_hash',
    'open_id', 'link_id', 'click_hash', 'track_id',
    'eid', 'rid', 'sid', 'cid', 'mid', 'lid',  # Short form IDs
    'token', 'signature', 'auth_token', 'verification',
    'unsubscribe_token', 'opt_out', 'preferences_token',
    
    # Additional newsletter/media tracking
    'newsletter', 'bulletin', 'digest', 'alert',
    'notification', 'update', 'promo', 'offer',
    'social_ref', 'email_ref', 'newsletter_ref',
    'source_code', 'promo_code', 'offer_code',
    'tracking_code', 'reference_code', 'campaign_code'
}
...
def url_cleaner(url, verbose, list_params):
    ...
    if not url:
        click.echo("Error: URL argument is required unless using --list-params", err=True)
        return
