...
def dd(folder, dry_run):
    ...
    for file_hash, files in duplicates.items():
        ...
        for duplicate_file, mtime, size in duplicates_to_move:
            ...
            if dry_run:
                click.echo(click.style(f"  WOULD MOVE: {duplicate_file} -> {dest_path}", fg="yellow"))
            else:
                try:
                    shutil.move(duplicate_file, dest_path)
                    click.echo(click.style(f"  MOVED: {duplicate_file} -> {dest_path}", fg="blue"))
                except OSError as e:
                    click.echo(click.style(f"  ERROR moving {duplicate_file}: {e}", fg="red"))
    
    # Summary
    click.echo(f"\nSummary:")
    if dry_run:
        click.echo(f"  Would move {total_duplicates} duplicate files")
        click.echo(f"  Would save {total_size_saved:,} bytes ({total_size_saved/1024/1024:.1f} MB)")
    else:
        click.echo(f"  Moved {total_duplicates} duplicate files")
        click.echo(f"  Saved {total_size_saved:,} bytes ({total_size_saved/1024/1024:.1f} MB)")
